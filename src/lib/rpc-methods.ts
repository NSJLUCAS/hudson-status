...
import type { DynamicSummary, StaticData, TaskQueryCondition, TaskQueryResult } from './nodeget-types';

export const listAgentUuids = (c: RpcClient) =>
  c.call<{ uuids?: string[] }>('nodeget-server_list_all_agent_uuid', {}).then(
    (r) => r?.uuids || []
  );
...
export const kvGetMulti = (
  ...
  c.call<{ namespace: string; key: string; value: unknown }[]>('kv_get_multi_value', {
    namespace_key: items,
  });
